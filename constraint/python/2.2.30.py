# 2.2.30: TEN + TEN + NINE + EIGHT + THREE = FORTY
from constraint import AllDifferentConstraint, Problem


def value(word, assignment):
    total = 0
    for letter in word:
        total = total * 10 + assignment[letter]
    return total


def solve():
    letters = ['E', 'F', 'G', 'H', 'I', 'N', 'O', 'R', 'T', 'Y']
    leading = ['E', 'F', 'N', 'T']
    problem = Problem()
    problem.addVariables(letters, range(10))
    problem.addConstraint(AllDifferentConstraint(), letters)
    for letter in leading:
        problem.addConstraint(lambda digit: digit != 0, [letter])

    def equation(*digits):
        assignment = dict(zip(letters, digits))
        return sum(value(word, assignment) for word in ['TEN', 'TEN', 'NINE', 'EIGHT', 'THREE']) == sum(value(word, assignment) for word in ['FORTY'])

    problem.addConstraint(equation, letters)
    return problem.getSolutions()


def main():
    for solution in solve():
        print('TEN + TEN + NINE + EIGHT + THREE = FORTY', solution)


if __name__ == "__main__":
    main()
